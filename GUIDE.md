...
## ⚠ Vercel Hobby(무료) 배포 한도 관리 (매우 중요)

**한도**: 팀당 **100 deployments / 하루** (UTC 자정 리셋 · 한국 오전 9시)
**프로젝트 구성**: `cookies` (shop) + `cookies-adm` (adm) 2개 → **커밋 1회 = 2 배포**

### 계산 감각
- 하루 50 커밋 이하 유지 필수 (커밋 × 2 = 배포)
- 실제로는 오늘(2026-08-02) 63커밋 push → 126 배포 시도 → 배포 큐 초과로 일부 skip 발생

### 실무 준수 규칙

1. **커밋 몰아서 push**
   - 여러 수정을 로컬에서 모아뒀다가 하루 끝에 한 번에 push
   - `git commit`은 자주 해도 되지만 `git push`는 몰아서
   - 잘게 나뉜 커밋은 로컬 정리 후 `git rebase -i`로 squash 가능

2. **테스트 커밋 금지**
   - `t · test · test2` 같은 실험용 커밋 push 금지 (로컬에서만 확인)
   - Vercel dev preview로 확인 필요하면 최소 1회로 끝낼 것

3. **브랜치별 자동배포 조정 (권장)**
   - Vercel Dashboard → 각 프로젝트 Settings → Git → Ignored Build Step
   - 실무: `real` 브랜치만 자동배포 · `dev`는 필요할 때만 수동 Redeploy
   - 세팅 5분 · 배포 절감 큼

4. **한도 초과 시 대응**
   - Vercel Deployments 탭에서 "Deployment limit reached" 확인
   - 대기: UTC 자정(한국 오전 9시) 지나면 자동 재개
   - 급할 땐 Pro $20/월 업그레이드 (즉시 무제한)

5. **한 사이클에서 관련 수정 병합**
   - 히어로 문구 · 폰트 · 색감 등 관련된 여러 수정 → 한 커밋으로 묶기
   - "여러 커밋으로 나눠야 리뷰 편함"보다 "배포 절감" 우선

### 위반 시 결과
- 실서비스 반영 지연 (하루 대기)
- 사장님이 급한 수정 필요할 때 반영 불가 → 판석이형 검수 지연 등 비즈니스 임팩트

### 재발 방지 체크리스트 (매 push 전)
- [ ] 지금 push할 커밋 개수 세보기 (2 프로젝트 × 커밋 = 배포)
- [ ] Vercel 대시보드 배포 카운터 확인 (90 넘으면 대기)
- [ ] 관련 수정 몰려있으면 rebase로 squash
- [ ] 확실히 확정된 코드만 push · 실험은 로컬에서만
