...
export interface ShopUiConfig {
  ...
  mainTop: {
    ...
    /** 로고 · 브랜드 워드마크 + 태그라인 (색상은 WYSIWYG 마커로 표현) */
    logo: {
      brand: string;
  ...
  footer: {
    ...
    /** 휴무 안내 (한국어) */
    closedKo: string;
    /** 대표자명 · 라벨(CEO)은 고정 · 값만 · 빈 값이면 shop에서 「―」 표시 */
    ceo: string;
    /** 사업자등록번호 · 빈 값이면 「―」 */
    bizNo: string;
    /** 주소 · 빈 값이면 「―」 */
...
export const DEFAULT_CONFIG: ShopUiConfig = {
  ...
  mainTop: {
    ...
    hero: {
      ...
      body: {
        ko: "이용 편의를 위해 [c:#C8907F]**2만엔 이상 구매 시 배송비와 통관보장 비용을 무료**[/c]로 제공합니다.\n\n이에 따라 일부 온라인 상품의 판매 가격이 소폭 조정됩니다.\n\n매장에 직접 방문하시는 고객님께는 기존 매장 가격 그대로 판매됩니다.",
        ja: "ご利用の便宜のため、[c:#C8907F]**2万円以上のご購入で送料と通関保証費用を無料**[/c]でご提供いたします。\n\nこれに伴い、一部のオンライン商品の販売価格が若干調整されます。\n\n店舗に直接お越しのお客様には、従来通り店舗価格にて販売しております。",
      },
      footer: {
        ko: "항상 감사합니다.",
